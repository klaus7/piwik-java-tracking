 ...
 * 
 * @link http://piwik.org/docs/tracking-api/
 * @author Martin Fochler, Klaus Pfeiffer, Bernhard Friedreich, Hendrik Stein
 */
public class SimplePiwikTracker implements PiwikTracker {
    ...
    /** The event category. (eg. Videos, Music, Games...) */
    private String eventCategory;

    /** The event action. (eg. Play, Pause, Duration, Add Playlist, Downloaded, Clicked...) */
    ...
    /**
     * Sets information to the tracker from the request. the information pageurl, urlreferer, useragend, ip, language
     * and the piwik cookie will be read.
     * 
     * @param request
     * @throws PiwikException if the urls read could not be parsed to an url object
     */
     ...
     * its context. If this is not wanted, create an own url object and use the equivalent function to this.
     * 
    ...
    /**
     * Sets the piwik cookie of the requester. Therefor the name of the cookie has to be 'piwik_visitor'. All other
     * cookies and null as parameter will reset the cookie.
     * 
     ...
     * 
     * The given string should be in the format of RFC2396. The string will be converted to an url with the apiurl as
     * its context. This will makes relative urls to the apiurl possible. If this is not wanted, create an own url
     * object and use the equivalent function to this.
     * 
    ...
    /**
     * Sets the user agent identification of the requester. This will be used to determine with which kind of client the
     * request was send.
     * 
    ...
    /**
     * Sets page custom variables; ignoring fixed order (differs from PHP version). still the order shouldn't change
     * anyway.
     * 
     * @param name Custom variable name
     * @param value Custom variable value
     * @return the count of the custom parameters
     * @throws PiwikException when the maximum size of variables is reached or the name or the value is longer as the
     *         maximum variable length
     */
    public final int setPageCustomVariable(final String name, final String value) throws PiwikException {
        if (!this.pageCustomVar.containsKey(name)
                && this.pageCustomVar.size() >= SimplePiwikTracker.MAX_CUSTOM_VARIABLE_LENGTH) {
    ...
    /**
     * Sets visitor custom variables; ignoring fixed order (differs from PHP version). still the order shouldn't change
     * anyway.
     * 
     * @param name Custom variable name
     * @param value Custom variable value
    ...
    public final int setVisitorCustomVariable(final String name, final String value) throws PiwikException {
        if (!this.visitorCustomVar.containsKey(name)
                && this.visitorCustomVar.size() >= SimplePiwikTracker.MAX_CUSTOM_VARIABLE_LENGTH) {
            throw new PiwikException("Max size of custom variables are reached. You can only put up to "
                    + SimplePiwikTracker.MAX_CUSTOM_VARIABLE_LENGTH + " custom variables to a request.");
        }
    ...
    /**
     * Adds a browser plugin to the list to detected plugins. With the boolean flag is set whether the plugin is enabled
     * or disabled.
     * 
    ...
    /**
     * Sets the event name. (eg. a Movie name, or Song name, or File name...).
     * 
    ...
    /**
     * Sets the event value. Must be a float or integer value (numeric), not a string.
     * 
     * @param eventValue the event value
     */
    public void setEventValue(final Float eventValue) {
    ...
    /**
     * See the equivalent function. Will call this function with ignoreNull set to be <code>true</code>.
     * 
     ...
     * @return the new query as a result of the root query with the new parameter and the value
     */
    ...
    /**
     * Adds a parameter to a given query and returns the full query. If the given value is <code>null</code> the added
     * query will be the string representation of <code>null</code> and NOT the empty string. If the given name is
     * <code>null</code>, the value will be added as a single parameter. Only if both name and value are
     * <code>null</code> the function will return the root query unmodified.
     * 
     * @param rootQuery the root query the new parameter will be added as string object
     * @param name the name of the parameter as string object
     * @param value the value ot the parameter as string object
